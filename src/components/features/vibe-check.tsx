...
import { X, Heart, Sparkles, Plane, ChevronLeft, ChevronRight, Wand2, Fingerprint, MapPin, ArrowRight, Loader2, Moon, LogIn } from "lucide-react"
...
export function VibeCheck({ isOpen, onClose }: VibeCheckProps) {
    ...
    // Detect user's origin city on mount
    useEffect(() => {
        async function detectLocation() {
            try {
                const location = await detectUserLocation()
                setOriginCity(location.city)
                setOriginCityCode(location.cityCode)
            } catch (error) {
                // Fallback to New York
                setOriginCity('New York')
                setOriginCityCode('JFK')
            } finally {
    ...
    // LocalStorage key for pending vibe data
    const PENDING_VIBE_KEY = 'safar_pending_vibe_data'

    // Auto-save pending vibe data when user logs in
    useEffect(() => {
        const savePendingVibeData = async () => {
            if (!user) return

            const pendingData = localStorage.getItem(PENDING_VIBE_KEY)
            if (!pendingData) return

            try {
                const { archetype, scores: savedScores, totalSwipes } = JSON.parse(pendingData)

                const { error } = await supabase.from('travel_profiles').upsert({
                    user_id: user.id,
                    archetype,
                    traits: { scores: savedScores, total_swipes: totalSwipes }
                }, { onConflict: 'user_id' })

                if (!error) {
                    localStorage.removeItem(PENDING_VIBE_KEY)
                    toast.success("Travel DNA Saved!", {
                        description: `Welcome back! Your ${archetype} profile is now saved.`,
                        duration: 4000
                    })
                    // If they just logged in and are still on the passport screen, proceed to pivot
                    if (complete && !showPivot && !showDeals) {
                        setShowPivot(true)
                    }
                }
            } catch (e) {
                console.error('Error saving pending vibe data:', e)
            }
        }

        savePendingVibeData()
    }, [user, complete, showPivot, showDeals, supabase])
    ...
    const saveAndClose = async () => {
        ...
        try {
            ...
            if (!user) {
                // Store results in localStorage for later
                const pendingData = {
                    archetype: finalArchetype,
                    scores,
                    totalSwipes: vibeCards.length,
                    savedAt: new Date().toISOString()
                }
                localStorage.setItem(PENDING_VIBE_KEY, JSON.stringify(pendingData))
            ...
            // Clear any pending data since we just saved
            localStorage.removeItem(PENDING_VIBE_KEY)
    ...
    // Continue without saving (for non-logged in users who want to explore)
    const continueWithoutSaving = () => {
        setShowLoginPrompt(false)
        setShowPivot(true)
        toast.info("Exploring as guest", {
            description: "Login anytime to save your Travel DNA!",
            duration: 3000
        })
    }
                                ...
                                <div className="space-y-4">
                                    <div className="space-y-2">
                                        ...
                                        <div className="relative group">
                                            {isDetectingLocation ? (
                                                <Loader2 className="absolute left-4 top-1/2 -translate-y-1/2 size-4 text-emerald-500 animate-spin" />
                                            ) : (
                                            ...
                                            <input
                                                type="text"
                                                value={isDetectingLocation ? "" : originCity}
                                                onChange={(e) => {
                                                ...
                                                placeholder={isDetectingLocation ? "Detecting your location..." : "Home City or Airport Code"}
                                                disabled={isDetectingLocation}
                                                className="w-full bg-white/5 border border-white/10 rounded-xl pl-12 pr-4 py-3 text-sm text-white placeholder:text-white/30 focus:outline-none focus:ring-1 focus:ring-emerald-500/50 transition-all font-mono disabled:opacity-50"
                                            />
                                        </div>
                                    </div>
            ...
            <AnimatePresence>
                {showLoginPrompt && (
                    ...
                    >
                        ...
                        >
                            <div className="flex items-center gap-3 mb-6">
                                <div className="size-12 rounded-xl bg-[#D4AF37]/10 flex items-center justify-center">
                                    <Fingerprint className="size-6 text-[#D4AF37]" />
                                </div>
                                <div>
                                    <h2 className="text-xl font-bold text-white">Save Your Travel DNA</h2>
                                    <p className="text-sm text-white/50">Your {getArchetype()} profile is ready!</p>
                                </div>
                            </div>

                            <p className="text-white/70 text-sm mb-6">
                                Login or create an account to save your Travel DNA and unlock personalized trip recommendations, curated deals, and your digital passport.
                            </p>
                            ...
                            <div className="space-y-3">
                                ...
                                <Button
                                    variant="outline"
                                    className="w-full h-12 bg-transparent border-white/20 text-white hover:text-white hover:bg-white/10 font-medium text-sm rounded-lg"
                                ...
                                <p className="text-center text-[10px] text-white/30 pt-2">
                                    Your results are saved locally and will sync when you login
                                </p>
                            </div>
                        </motion.div>
                    </motion.div>
